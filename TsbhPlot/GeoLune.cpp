...
void GeoLune::draw(std::vector<osg::Vec3d> points)
{
...
std::vector<osg::Vec2> GeoLune::calculateParts(const std::vector<osg::Vec2> &ctrlPts)
{
	...
	//至少需要三个控制点
	std::vector<osg::Vec2> points;		//add by gkk 20220216
    if (ctrlPts.size() > 2) {
        osg::Vec2 pointA = controlPoints[0];
        osg::Vec2 pointB = controlPoints[1];
        osg::Vec2 pointC = controlPoints[2];
        //std::vector<osg::Vec2> points;			//保证所有路径都能有返回值，Note by gkk 20220216
	...
	return points;		//保证所有路径都能有返回值，此处的points值应该为空的，要注意以下！Add by gkk 20220216
}
