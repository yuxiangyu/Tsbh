...
SimpleBoxDrawable::SimpleBoxDrawable(const osg::BoundingBox & box, int type,const osgEarth::BBoxSymbol & bboxSymbol) :osg::Geometry()
{
	...
	osg::ref_ptr<osg::Image> img = osgDB::readImageFile("resources/textures/biaopai_mini.png");
	if (img.valid())
	{
		tex->setImage(img);
	}
...
SimpleBoxDrawable::SimpleBoxDrawable(const osg::BoundingBox & box, int type ,float margin) :osg::Geometry()
{
	setUseVertexBufferObjects(true);
	...
	if (type == 1)
	{
		 img = osgDB::readImageFile("resources/textures/info1.png");
	}
	else 
	{
		 img = osgDB::readImageFile("resources/textures/biaopai_mini.png");
	}
	if (img.valid())
	{
		tex->setImage(img);
...
SimpleBoxDrawable::SimpleBoxDrawable(const osg::BoundingBox & box, int type, float marginTop, float marginBottom, float marginLeft, float marginRight)
{
	setUseVertexBufferObjects(true);
	...
	tex->setFilter(osg::Texture::MIN_FILTER, osg::Texture::LINEAR);
	tex->setFilter(osg::Texture::MAG_FILTER, osg::Texture::LINEAR);
	osg::ref_ptr<osg::Image> img = osgDB::readImageFile("resources/textures/info1.png");
	if (img.valid())
	{
		tex->setImage(img);
	}
