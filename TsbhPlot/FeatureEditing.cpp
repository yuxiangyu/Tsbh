...
public:
    MoveFeatureDraggerCallback(FeatureNode* featureNode, int point):
      ...
      virtual void onPositionChanged(const Dragger* sender, const osgEarth::GeoPoint& position)
      {
		  ...
		  osg::Referenced* pro = const_cast<osg::Referenced*>(_featureNode->getUserData());
		  ...
		  if (pro) {
			  ...
			  {
				  points.push_back((*feature->getGeometry())[i]);
			  }
			  osg::Vec3d lla, xyz = _featureNode->getBound().center();
			  _featureNode->getMapNode()->getMap()->getProfile()->getSRS()->getEllipsoid()->convertXYZToLatLongHeight(xyz.x(), xyz.y(), xyz.z(), lla.y(), lla.x(), lla.z());
			  lla.x() = osg::RadiansToDegrees(lla.x());
			  lla.y() = osg::RadiansToDegrees(lla.y());

			  toolpro->getTextNode()->setPosition(osgEarth::GeoPoint::GeoPoint(_featureNode->getMapNode()->getMapSRS(), lla));

			  toolpro->getSignageNode()->setPosition(osgEarth::GeoPoint::GeoPoint(_featureNode->getMapNode()->getMapSRS(), lla));

			  toolpro->setCenterPoint(_featureNode->getBound().center());
...
FeatureEditor::init()
{
	...
	qDebug() << feature->getGeometry()->size();
