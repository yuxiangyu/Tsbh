...
Coordinate::Coordinate(CoordinateSystem system, const SimVec3 &pos, double elapsedECITime)
  : system_(system),
...
Coordinate::Coordinate(CoordinateSystem system, const SimVec3 &pos, const SimVec3 &ori, double elapsedECITime)
  : system_(system),
  elapsedEciTime_(elapsedECITime),
  hasVel_(false),
  hasOri_(false),
...
Coordinate::Coordinate(CoordinateSystem system, const SimVec3 &pos, const SimVec3 &ori, const SimVec3 &vel, double elapsedECITime)
  : system_(system),
  elapsedEciTime_(elapsedECITime),
  hasVel_(false),
  hasOri_(false),
...
Coordinate::Coordinate(CoordinateSystem system, const SimVec3 &pos, const SimVec3 &ori, const SimVec3 &vel, const SimVec3 &acc, double elapsedECITime)
  : system_(system),
  elapsedEciTime_(elapsedECITime),
  hasVel_(false),
  hasOri_(false),
...
{
  assert(_finite(x) && _finite(y) && _finite(z));
  pos_.set(x, y, z);
}
...
{
  assert(_finite(lat) && _finite(lon) && _finite(alt));
  pos_.set(lat, lon, alt);
}

/// Sets the position state vector
void Coordinate::setPosition(const SimVec3 &pos)
{
  assert(_finite(pos.x()) && _finite(pos.y()) && _finite(pos.z()));
  pos_.set(pos);
}
...
{
  assert(_finite(yaw) && _finite(pitch) && _finite(roll));
  hasOri_ = true;
...
{
  assert(_finite(psi) && _finite(theta) && _finite(phi));
  hasOri_ = true;
  ori_.set(psi, theta, phi);
}

/// Sets the orientation state vector
void Coordinate::setOrientation(const SimVec3 &ori)
{
  assert(_finite(ori.yaw()) && _finite(ori.pitch()) && _finite(ori.roll()));
  hasOri_ = true;
  ori_.set(ori);
}
...
void Coordinate::setVelocity(const SimVec3 &vel)
{
  assert(_finite(vel.x()) && _finite(vel.y()) && _finite(vel.z()));
  hasVel_ = true;
  vel_.set(vel);
}
...
void Coordinate::setAcceleration(const SimVec3 &acc)
{
  assert(_finite(acc.x()) && _finite(acc.y()) && _finite(acc.z()));
  hasAcc_ = true;
  acc_.set(acc);
}
...
CoordinateConverter::CoordinateConverter()
  ...
  invLatRadius_(0.0),
  invLonRadius_(0.0),
  referenceOrigin_(1e+300, 1e+300, 0.0),
...
const SimVec3& CoordinateConverter::referenceOrigin() const
{
 // if (!hasReferenceOrigin())
...
void CoordinateConverter::calculateReferenceRadius_(const SimVec3 &lla)
{
  ...
  refOriginStatus_ = (TMath::areEqual(fabs(referenceOrigin_[0]), M_PI_2, 1e-5)) ? REF_ORIGIN_SCALED_FLAT_EARTH_DEGENERATE : REF_ORIGIN_SET;

  // prevent divide by zero errors
  assert(latRadius_ != 0);
  assert(lonRadius_ != 0);
  invLatRadius_ = (TMath::areEqual(latRadius_, 0, 1e-5)) ? std::numeric_limits<float>::max() : 1.0 / latRadius_;
  // at the pole, the radius of curvature will be infinite
  invLonRadius_ = (TMath::areEqual(lonRadius_, 0, 1e-5)) ? std::numeric_limits<float>::max() : 1.0 / lonRadius_;
...
{
  calculateReferenceRadius_(SimVec3(lat*DEG2RAD, lon*DEG2RAD, alt));
}

void CoordinateConverter::setReferenceOriginDegrees(const SimVec3 &lla)
{
  calculateReferenceRadius_(SimVec3(lla.lat()*DEG2RAD, lla.lon()*DEG2RAD, lla.alt()));
}

void CoordinateConverter::setReferenceOrigin(double lat, double lon, double alt)
{
  calculateReferenceRadius_(SimVec3(lat, lon, alt));
}

void CoordinateConverter::setReferenceOrigin(const SimVec3 &lla)
{
...
void CoordinateConverter::applyTPOffsetRotate_(Coordinate &tpCoord) const
{
  ...
  {
	  SimVec3 eul(tpCoord.orientation());
  ...
  {
	  SimVec3 acc(tpCoord.acceleration());
...
void CoordinateConverter::reverseTPOffsetRotate_(Coordinate &gtpCoord) const
{
  ...
  {
	  SimVec3 eul(gtpCoord.orientation());
    eul.setPsi(angFix2PI(eul.psi() + tangentPlaneRotation_));
  ...
  {
	  SimVec3 acc(gtpCoord.acceleration());
    const double tmpAx = acc.x();
    acc.setX(tmpAx*cosTPR_ + acc.y()*sinTPR_);
    // Note: acc.setX() changes the .x() return value, changing the call below.  Use a temp variable (tmpAx)
    acc.setY(-tmpAx*sinTPR_ + acc.y()*cosTPR_);
...
int CoordinateConverter::convertGeodeticToFlat_(const Coordinate& llaCoord, Coordinate& flatCoord, CoordinateSystem system) const
{
  ...
  {
    const SimVec3& llaPos(llaCoord.position());
...
int CoordinateConverter::convertFlatToGeodetic_(const Coordinate& flatCoord, Coordinate& llaCoord) const
{
 ...
 //   SIM_ERROR << "convertFlatToGeodetic, invalid local coordinate system: " << __LINE__ << std::endl;
  ...
  {
	  SimVec3 nedPos(flatCoord.position());
    // meters (North East Down system)
    // +X is North, Latitude is North-South
  ...
  {
	  SimVec3 enuPos(flatCoord.position());
    // meters (East North Up system)
    // +X is East, Longitude is East-West
  ...
  {
	  SimVec3 nwuPos(flatCoord.position());
    // meters (North West Up system)
    // +X is North, Latitude is North-South
...
int CoordinateConverter::convertXEastToEcef_(const Coordinate &tpCoord, Coordinate &ecefCoord) const
{
 ...
 //   SIM_ERROR << "convertXEastToEcef, input system is not XEAST: " << __LINE__ << std::endl;
  ...
  TMath::d3MTv3Mult(rotationMatrixENU_, tpCoord.position(), pos);

  // apply translation to earth center origin
  SimVec3 ecefPos;
  TMath::v3Add(pos, tangentPlaneTranslation_, ecefPos);
  ecefCoord.setPosition(ecefPos);

  if (tpCoord.hasVelocity())
  {
	  SimVec3 ecefVel;
	  TMath::d3MTv3Mult(rotationMatrixENU_, tpCoord.velocity(), ecefVel);
  ...
  {
	  SimVec3 ecefAcc;
	  TMath::d3MTv3Mult(rotationMatrixENU_, tpCoord.acceleration(), ecefAcc);
  ...
  if (tpCoord.hasOrientation())
  {
    ...
    double BL[3][3];
	TMath::d3EulertoDCM(tpCoord.orientation(), BL);

    // calculate Body to Earth rotation matrix fBE
    double BE[3][3];
	TMath::d3MMmult(BL, rotationMatrixNED_, BE);

    // calculate Euler angles for platform body in ECEF coordinates
	SimVec3 ecefEul;
	TMath::d3DCMtoEuler(BE, ecefEul);
...
int CoordinateConverter::convertEcefToXEast_(const Coordinate &ecefCoord, Coordinate &tpCoord) const
{
  ...
  TMath::v3Subtract(ecefCoord.position(), tangentPlaneTranslation_, pos);

  // rotate to X-East
  SimVec3 tpPos;
  TMath::d3Mv3Mult(rotationMatrixENU_, pos, tpPos);
  tpCoord.setPosition(tpPos);

  if (ecefCoord.hasVelocity())
  {
	  SimVec3 tpVel;
	  TMath::d3Mv3Mult(rotationMatrixENU_, ecefCoord.velocity(), tpVel);
  ...
  {
	  SimVec3 tpAcc;
	  TMath::d3Mv3Mult(rotationMatrixENU_, ecefCoord.acceleration(), tpAcc);
  ...
  if (ecefCoord.hasOrientation())
  {
    ...
    double BE[3][3];
	TMath::d3EulertoDCM(ecefCoord.orientation(), BE);

    // multiply BE * (LE) transpose = BL (Body to Local Topo rotation matrix)
    double BL[3][3];
	TMath::d3MMTmult(BE, rotationMatrixNED_, BL);

    // get local Eulers
	SimVec3 tpEul;
	TMath::d3DCMtoEuler(BL, tpEul);
...
void CoordinateConverter::swapNedEnu(const SimVec3 &inVec, SimVec3 &outVec)
{
...
void CoordinateConverter::swapNedNwu(const SimVec3 &inVec, SimVec3 &outVec)
{
...
void CoordinateConverter::convertEnuToNwu(const SimVec3 &inVec, SimVec3 &outVec)
{
  outVec.set(inVec.y(), -inVec.x(), inVec.z());
}
...
void CoordinateConverter::convertNwuToEnu(const SimVec3 &inVec, SimVec3 &outVec)
{
  outVec.set(-inVec.y(), inVec.x(), inVec.z());
}
...
int CoordinateConverter::convertGeodeticToEcef(const Coordinate &llaCoord, Coordinate &ecefCoord, LocalLevelFrame localLevelFrame)
{
  ...
  if (llaCoord.hasOrientation())
  {
    ...
    // calculate Body to Local rotation matrix using Local Eulers
	TMath::d3EulertoDCM(llaCoord.orientation(), BL);

    // calculate Body to Earth rotation matrix fBE
	TMath::d3MMmult(BL, LE, BE);

    // calculate Euler angles for platform body in ECEF coordinates
	SimVec3 ecefOri;
	TMath::d3DCMtoEuler(BE, ecefOri);
  ...
  if (llaCoord.hasVelocity() || llaCoord.hasAcceleration())
  {
    ...
    if (llaCoord.hasVelocity())
    {
	  ...
	  SimVec3 ecefVel;
	  TMath::d3MTv3Mult(LE, nedVec, ecefVel);
      ecefCoord.setVelocity(ecefVel);
    }
    ...
    if (llaCoord.hasAcceleration())
    {
	  ...
	  SimVec3 ecefAcc;
	  TMath::d3MTv3Mult(LE, nedVec, ecefAcc);
      ecefCoord.setAcceleration(ecefAcc);
    }
  }
  return 0;
...
int CoordinateConverter::convertEcefToGeodetic(const Coordinate &ecefCoord, Coordinate &llaCoord, LocalLevelFrame localLevelFrame)
{
  ...
  if (ecefCoord.hasOrientation())
  {
    ...
    // Multiply BE * (LE) transpose = BL (Body to Local Topo rotation matrix)
	TMath::d3MMTmult(BE, LE, BL);

    // get local Eulers
	SimVec3 llaOri;
	TMath::d3DCMtoEuler(BL, llaOri);
  ...
  if (ecefCoord.hasVelocity() || ecefCoord.hasAcceleration())
  {
    ...
    if (ecefCoord.hasVelocity())
    {
		TMath::d3Mv3Mult(LE, ecefCoord.velocity(), nedVec);
      if (localLevelFrame == LOCAL_LEVEL_FRAME_NED)
      {
		  SimVec3 llaVel;
    ...
    if (ecefCoord.hasAcceleration())
    {
		TMath::d3Mv3Mult(LE, ecefCoord.acceleration(), nedVec);
      if (localLevelFrame == LOCAL_LEVEL_FRAME_NED)
      {
		  SimVec3 llaAcc;
...
void CoordinateConverter::convertEciEcef_(const Coordinate &inCoord, Coordinate &outCoord)
{
  ...
  const SimVec3& inPos(inCoord.position());
  ...
  if (inCoord.hasVelocity())
  {
    ...
    const SimVec3& inVel(inCoord.velocity());
    ...
    if (inCoord.hasAcceleration())
    {
      ...
      const SimVec3& inAcc(inCoord.acceleration());
...
int CoordinateConverter::convertEcefToGeodeticPos(const SimVec3 &ecefPos, SimVec3 &llaPos)
{
  ...
  const double p = sqrt(TMath::square(ecefPos.x()) + TMath::square(ecefPos.y()));
  ...
  assert(S == 0.0 || TMath::sign(S) == TMath::sign(Cc));
  const double lat = TMath::sign(ecefPos.z()) * atan(S / Cc);
...
///@pre ecefPos valid
void CoordinateConverter::convertGeodeticPosToEcef(const SimVec3 &llaPos, SimVec3 &ecefPos, const double semiMajor, const double eccentricitySquared)
{
  // convert lat, lon, alt to ECEF geocentric using WGS84 ellipsoidal earth model
  const double sLat = sin(llaPos.lat());
  const double Rn = semiMajor / sqrt(1.0 - eccentricitySquared * TMath::square(sLat));
...
void CoordinateConverter::convertEcefToGeodeticVel(const SimVec3 &llaPos, const SimVec3 &ecefVel, SimVec3 &llaVel, LocalLevelFrame localLevelFrame)
{
  // calculate Local To Earth rotation matrix at lat, lon position of input platform
  // (orientation is translated to geocentric Eulers based on the transformation from a local
  // tangent plane coordinate system at the lat and lon of the platform)
  ...
  // is done using transformation to a tangent plane at the lat, lon of the platform)
  SimVec3 nedVector;
  TMath::d3Mv3Mult(LE, ecefVel, nedVector);
...
void CoordinateConverter::convertEcefToGeodeticOri(const SimVec3 &llaPos, const SimVec3 &ecefOri, SimVec3 &llaOri, LocalLevelFrame localLevelFrame)
{
  // calculate Local To Earth rotation matrix at lat, lon position of input platform
  // (orientation is translated to geocentric Eulers based on the transformation from a local
  // tangent plane coordinate system at the lat and lon of the platform)
...
void CoordinateConverter::convertEcefToGeodeticAccel(const SimVec3 &llaPos, const SimVec3 &ecefAcc, SimVec3 &llaAcc, LocalLevelFrame localLevelFrame)
{
  // calculate Local To Earth rotation matrix at lat, lon position of input platform
  // (orientation is translated to geocentric Eulers based on the transformation from a local
  // tangent plane coordinate system at the lat and lon of the platform)
  ...
  // is done using transformations to a tangent plane at the lat, lon of the platform)
  SimVec3 nedVector;
  TMath::d3Mv3Mult(LE, ecefAcc, nedVector);
...
void CoordinateConverter::convertGeodeticOriToEcef(const SimVec3 &llaPos, const SimVec3 &llaOri, SimVec3 &ecefOri, LocalLevelFrame localLevelFrame)
{
