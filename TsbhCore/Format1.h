...
public :
	template <class T>
	static std::string join(const std::vector<T>& params, const std::string& delimiter)
	{
		std::stringstream s;
	...
	*/
	static size_t stringCaseFind(const std::string &str1, const std::string &str2);
	...
	*/
	//RTSSCORE_EXPORT bool getStrippedLine(std::istream& is, std::string& str);
	...
	*/
	static std::string getExtension(const std::string &inName, bool toLower = true);
	...
	*/
	static bool hasExtension(const std::string& inName, std::string newExt);
	 ...
	 */
	 /*RTSSCORE_EXPORT std::string buildString(const std::string& prefix, double value, size_t width=11,
